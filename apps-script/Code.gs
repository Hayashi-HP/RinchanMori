...
function doGet(e) {
  ...
  const setupInfo = setupProject(ss);
  if (action === 'setup') return jsonOutput({ ok: true, action: action, setup: setupInfo, version: VERSION });
  ...
  return jsonOutput({ ok: true, app: 'RinchanMori', version: VERSION, setup: setupInfo, message: 'Apps Script is running.' });
}
...
function doPost(e) {
  try {
    ...
    if (action === 'myThanks') {
      ...
      return jsonOutput({ ok: true, action: action, thanks: thanks, version: VERSION });
    }
...
function setupProject(ss) {
  ...
  ensureSheet(ss, SHEET_THANKS, ['thanksId','fromParticipantId','fromName','toParticipantId','toName','toDept','reason','createdAt','version','savedAt']);
...
function saveActivity(ss, data) {
  ...
  const values = [activityId, data.participantId || data.id || '', data.deviceId || '', data.date || '', Number(data.steps || 0), data.challenge === true || data.challenge === 'true', data.comment || '', data.createdAt || '', data.version || data.appVersion || VERSION, new Date().toISOString()];
  if (row > 0) { sheet.getRange(row, 1, 1, values.length).setValues([values]); return { type: 'updated', row: row, activityId: activityId }; }
  sheet.appendRow(values);
...
function deleteActivity(ss, data) {
  ...
  if (!activityId) throw new Error('activity_id_required');
  const row = findRowByValue(sheet, 1, activityId);
  if (row > 0) { sheet.deleteRow(row); return { deleted: true, row: row, activityId: activityId }; }
...
function saveThanks(ss, data) {
  ...
  const now = new Date().toISOString();
  const values = [thanksId, data.fromParticipantId || '', data.fromName || '', data.toParticipantId || '', data.toName || '', data.toDept || '', data.reason || 'ありがとう', data.createdAt || now, data.version || data.appVersion || VERSION, now];
  if (row > 0) { sheet.getRange(row, 1, 1, values.length).setValues([values]); return { type: 'updated', row: row, thanksId: thanksId }; }
  sheet.appendRow(values);
  return { type: 'inserted', row: sheet.getLastRow(), thanksId: thanksId };
}

function getMyThanks(ss, data) {
  const id = normalizeEmployeeId(data.employeeId || data.id || data.toParticipantId || '');
  if (!id) return [];
  return readTable(ss.getSheetByName(SHEET_THANKS)).filter(function(t) { return normalizeEmployeeId(t.toParticipantId || '') === id; }).sort(function(a, b) { return String(b.createdAt || '').localeCompare(String(a.createdAt || '')); }).slice(0, 50);
}
...
function getDashboard(ss) {
  ...
  const members = Object.keys(byUser).map(function(id) { return byUser[id]; });
  return { generatedAt: new Date().toISOString(), totalUsers: members.length, totalActivities: activities.length, totalSteps: activities.reduce(function(sum, item) { return sum + Number(item.steps || 0); }, 0), totalThanks: thanks.length, members: members, ranking: rankMembers(members).slice(0, 20) };
}
...
function getAdminStats(ss) {
  ...
  const members = Object.keys(byUser).map(function(id) { return byUser[id]; });
  const deptMap = {}, monthMap = {}, csvRows = [];
  ...
  const deptRanking = Object.keys(deptMap).map(function(k) { const d = deptMap[k]; return { dept: d.dept, userCount: Object.keys(d.users).length, activityCount: d.activityCount, totalSteps: d.totalSteps }; }).sort(function(a, b) { return b.totalSteps - a.totalSteps; });
  const monthly = Object.keys(monthMap).map(function(k) { return monthMap[k]; }).sort(function(a, b) { return String(b.month).localeCompare(String(a.month)); });
  return { generatedAt: new Date().toISOString(), totalUsers: members.length, totalActivities: activities.length, totalSteps: activities.reduce(function(sum, item) { return sum + Number(item.steps || 0); }, 0), ranking: rankMembers(members), deptRanking: deptRanking, monthly: monthly, csvRows: csvRows };
}
