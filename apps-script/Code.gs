...
function doGet(e){
  ...
  if(action==='setup')return jsonOutput({ok:true,action,setup,version:VERSION});
  if(action==='departments')return jsonOutput({ok:true,action,departments:getDepartments(ss),version:VERSION});
  if(action==='dashboard')return jsonOutput({ok:true,action,data:getDashboard(ss),version:VERSION});
  return jsonOutput({ok:true,app:'RinchanMori',version:VERSION,setup,message:'Apps Script is running.'});
}
...
function doPost(e){
  try{
    ...
    const ss=SpreadsheetApp.getActiveSpreadsheet();
    setupProject(ss);
    if(action==='setup')return jsonOutput({ok:true,action,setup:setupProject(ss),version:VERSION});
    if(action==='departments')return jsonOutput({ok:true,action,departments:getDepartments(ss),version:VERSION});
    if(action==='dashboard')return jsonOutput({ok:true,action,data:getDashboard(ss),version:VERSION});
    if(action==='getUserState')return jsonOutput({ok:true,action,state:getUserState(ss,data),version:VERSION});
    if(action==='markNewsRead')return jsonOutput({ok:true,action,state:markNewsRead(ss,data),version:VERSION});
    if(action==='adminStats'){
      if(!isAdminRequest(ss,data))return jsonOutput({ok:false,error:'admin_required',version:VERSION});
    ...
    try{const ss=SpreadsheetApp.getActiveSpreadsheet();setupProject(ss);writeLog(ss,'error','','','ng',err.message);}catch(ignore){}
...
function setupProject(ss){
  const usersSheet=ensureSheet(ss,SHEET_USERS,['id','deviceId','name','dept','nick','declaration','weeklyGoal','createdAt','updatedAt','version','lastSavedAt','email','pin4','employeeId','admin','weeklyStepGoal']);
  ...
  seedDepartments(deptSheet);
  return{spreadsheetId:ss.getId(),sheets:ss.getSheets().map(s=>s.getName())};
}
...
function saveUser(ss,data){
  ...
  const values=[user.id,user.deviceId,user.name,user.dept,user.nick,user.declaration,user.weeklyGoal,user.createdAt,user.updatedAt,user.version,user.lastSavedAt,user.email,user.pin4,user.employeeId,user.admin,user.weeklyStepGoal];
  if(row>0){sheet.getRange(row,1,1,values.length).setValues([values]);return{type:'updated',row,id:employeeId,user:publicUser(user)};}
...
function getUserState(ss,data){const id=normalizeEmployeeId(data.employeeId||data.id||data.participantId||'');const user=getPublicUserById(ss,id);return{employeeId:id,user,readNewsIds:getReadNewsIds(ss,id),serverAt:new Date().toISOString()};}
