# RinchanMori v0.9.62

みんなで育てる、健康と笑顔の杜。

## v0.9.62 内容

- JavaScript の主要機能を `core/` と `features/` に完全移行
- 旧 `v***.js` 系の主要機能ファイルを削除
- `features/growth.js`、`features/chart.js`、`features/guide.js` を追加
- ホームの木の成長演出、週間歩数グラフ、りんちゃんガイドを `features/` へ移行
- `core/error-log.js` を追加し、主要画面で端末内エラーログを保存
- `pages/diagnostics.html` を追加し、通信・同期・キャッシュ・端末エラーを確認可能に変更
- 管理画面から診断画面を開ける導線を追加
- 全主要画面のキャッシュバージョンを `0962` に統一
- リポジトリ検索で旧JS名と旧キャッシュ参照が残っていないことを確認
- v1.0 RC に向けた品質確認フェーズへ移行

## JavaScript 構成

```text
js/
├── core/
│   ├── storage.js          # localStorage、JSON安全処理、端末ID、ユーザーデータ管理
│   ├── error-log.js        # 端末内JavaScriptエラーログ保存
│   ├── api.js              # Apps Script API 通信
│   ├── sync.js             # 差分同期、同期状態表示、サーバー状態反映
│   └── offline-queue.js    # 通信失敗時の未送信保存、自動再送
│
└── features/
    ├── auth.js             # ログイン、初回登録、ログアウト、プロフィール保存
    ├── activity.js         # 歩数保存、編集、削除、最近の記録
    ├── thanks.js           # ありがとう送信、受信、集計、タイムライン
    ├── news.js             # 通信、お知らせ、既読、通知バッジ
    ├── mori.js             # 杜、部署、木、杜レベル
    ├── mypage.js           # マイページ、バッジ、継続、目標表示
    ├── admin.js            # 管理画面、集計、利用者一覧
    ├── diagnostics.js      # 動作診断、端末情報、エラーログ表示
    ├── growth.js           # 木の成長演出、季節表示、バッジ演出
    ├── chart.js            # 週間歩数グラフ、週目標、先週比較
    └── guide.js            # 初回ガイド、チュートリアル
```

## Apps Script 構成

Google Apps Script 側にも、以下のファイル名で分けて作成してください。

```text
apps-script/
├── Code.gs       # API入口 doGet / doPost
├── Config.gs     # 定数・初期部署
├── Common.gs     # 共通関数
├── Setup.gs      # シート初期化・部署マスタ
├── User.gs       # ログイン・プロフィール・本人状態
├── Activity.gs   # 歩数登録・削除・履歴
├── Thanks.gs     # ありがとう保存・受信・送信・タイムライン
├── News.gs       # 通信既読
└── Admin.gs      # 管理画面・ランキング集計
```

## Apps Script 反映手順

1. Google スプレッドシートを開く
2. 拡張機能 → Apps Script を開く
3. 既存の `Code.gs` を GitHub の `apps-script/Code.gs` の内容に置き換える
4. `Config.gs`、`Common.gs`、`Setup.gs`、`User.gs`、`Activity.gs`、`Thanks.gs`、`News.gs`、`Admin.gs` を新規作成して、それぞれ GitHub の同名ファイルを貼り付ける
5. `setupProjectManual()` を一度実行
6. Web アプリを再デプロイ

## 同期方針

りんちゃんの杜では、正式データはスプレッドシートを正とします。

通常利用では、スマホ内のデータをキャッシュとして使い、画面はすぐ表示します。その後、裏側で Apps Script に接続し、スプレッドシート側の最新データで画面を更新します。

```text
アプリを開く
↓
手元キャッシュで即表示
↓
裏でサーバー同期
↓
最新データで必要箇所だけ更新
```

保存系APIは、保存が成功したあと `getUserState` 相当の最新状態を返します。画面側はその `state` を受け取り、以下をまとめて更新します。

- プロフィール
- 歩数履歴
- 受け取ったありがとう
- 送ったありがとう
- 通信タイムライン
- お知らせ既読状態
- ありがとう件数

ログアウト後に再ログインした場合も、`loginUser` と `getUserState` により同じ内容を復元します。

## 診断・保守

`pages/diagnostics.html` で以下を確認できます。

- `core/` モジュールの読み込み状態
- API URL 設定
- ログイン情報
- 歩数・ありがとう・未送信キューのキャッシュ件数
- syncToken
- オンライン状態
- 端末IDと保存キー
- 端末内JavaScriptエラーログ

`core/error-log.js` は、ホーム、ログイン、初回登録、歩数記録、通信、杜、マイページ、管理画面、診断画面で読み込みます。端末内の直近30件のJavaScriptエラーを保存し、診断画面から確認できます。

## 画面設計

- ホーム：りんちゃんの挨拶、今日の一歩、歩数記録導線、週間歩数、あなたの木、初回ガイド
- 杜：部署・グループ単位の木、個人へのありがとう送信
- 歩数記録：歩数保存、直近記録の修正・削除
- 通信：今日の杜、ありがとうの広がり、お知らせ、グループニュース
- マイページ：あなたの木、プロフィール、バッジ、ありがとう、最近の活動、目標、健康宣言
- 管理画面：利用状況、部署別集計、CSV確認、診断画面への導線
- 診断画面：端末・通信・同期・キャッシュ・エラーログ確認

## v1.0 RC 前チェック

詳細な確認項目は以下を使用してください。

- [v1.0 RC 前チェックリスト](docs/v1-rc-checklist.md)

主な確認観点は以下です。

- ログアウト後の再ログインで本人データが復元されること
- 別端末ログインで歩数、ありがとう、プロフィール、目標が反映されること
- オフライン保存後にオンライン復帰で未送信データが再送されること
- ホーム、歩数記録、通信、杜、マイページ、管理画面がエラーなく表示されること
- 診断画面で端末内エラーログと未送信キューを確認できること
- 画面表示が手元キャッシュで素早く開始し、裏同期後に最新化されること

## 今後の整理方針

- Apps Script 側へ端末エラーログ送信機能を追加
- 管理画面で全端末のエラーを一覧確認できるようにする
- API通信回数と再描画回数を削減
- CSS を共通・画面別へ整理
- v1.0 リリース候補として実機確認を進める
