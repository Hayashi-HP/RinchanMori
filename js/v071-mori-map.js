...
function sendThanksV073(id){if(hasThanksTodayV077(id)){alert('この木へのありがとうは今日は送信済みです。');return;}const data=v071ReadJson('rinchanThanks',{});data[id]=Number(data[id]||0)+1;v071SaveJson('rinchanThanks',data);const daily=v071ReadJson('rinchanThanksDaily',{});daily[id]=v071Today();v071SaveJson('rinchanThanksDaily',daily);const el=document.getElementById('thanksCount');if(el)el.textContent=data[id];const btn=document.getElementById('thanksButton');if(btn){btn.textContent='今日は送信済み';btn.disabled=true;}addThanksTimelineV090(window.rinchanSelectedTree);runRinchanDeliveryV074(window.rinchanSelectedTree);runThanksHeartV073();}
function addThanksTimelineV090(tree){const list=v071ReadJson('rinchanGoodTimeline',[]);const target=tree?(tree.dept||'杜の木'):'杜の木';list.unshift({id:'T'+Date.now().toString(36),icon:'❤️',title:'ありがとうが届きました',body:target+'の木に、応援の気持ちが届きました。',createdAt:new Date().toISOString()});v071SaveJson('rinchanGoodTimeline',list.slice(0,50));}
