...
function openTreeCardV071(u,level,rank,area){
  ...
  card.innerHTML='<button class="tree-card-close" onclick="closeTreeCardV071()">×</button><p class="label">杜の木'+rankText+'</p><div class="tree-card-icon">'+v071Tree(level)+'</div><h2>'+v071Mask(u.name||u.nick)+'</h2><p>'+((u.dept||'所属未設定'))+'・'+area+'</p><div class="mini-stats"><div><strong>Lv.'+level+'</strong><small>木レベル</small></div><div><strong>'+v071Num(u.totalSteps)+'</strong><small>歩</small></div><div><strong>'+v071Num(u.activityCount)+'</strong><small>記録</small></div></div><div class="thanks-reason-box"><p class="thanks-reason-title">理由を添える（任意）</p><div class="thanks-reason-options"><button type="button" onclick="selectThanksReasonV095(this,\'元気をもらった\')">😊 元気をもらった</button><button type="button" onclick="selectThanksReasonV095(this,\'助けてもらった\')">🤝 助けてもらった</button><button type="button" onclick="selectThanksReasonV095(this,\'頑張っていた\')">🌸 頑張っていた</button></div></div><button class="submit pill-button thanks-button" id="thanksButton" onclick="sendThanksV073(\''+id+'\')"'+buttonDisabled+'>'+buttonLabel+'</button><p class="thanks-count">受け取ったありがとう：<strong id="thanksCount">'+thanksCount+'</strong>件</p><p class="thanks-limit-note">ありがとうは同じ木に1日1回まで送れます。</p>';
