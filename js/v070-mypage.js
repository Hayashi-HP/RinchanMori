const RINCHAN_V070='v0.9.43';
const RINCHAN_MYPAGE_ACTIVITY_DAYS=30;
const RINCHAN_MYPAGE_ACTIVITY_LIMIT=10;
...
function v070IsWithinDays(value,days){if(!value)return false;const d=new Date(String(value).slice(0,10)+'T00:00:00');if(isNaN(d))return false;const base=new Date();base.setHours(0,0,0,0);const cutoff=new Date(base);cutoff.setDate(base.getDate()-(days-1));return d>=cutoff}
...
function renderV070Mypage(){
  ...
  renderHistoryV070(buildRecentActivityV139(list).slice(0,RINCHAN_MYPAGE_ACTIVITY_LIMIT));
...
function buildRecentActivityV139(steps){const stepItems=(steps||[]).filter(a=>v070IsWithinDays(a.date||a.createdAt,RINCHAN_MYPAGE_ACTIVITY_DAYS)).map(a=>({kind:'steps',date:a.date||a.createdAt||'',createdAt:a.createdAt||a.date||'',steps:Number(a.steps||0)}));const sent=(v070ReadJson('rinchanSentThanks',[])||[]).filter(t=>v070IsWithinDays(t.createdAt,RINCHAN_MYPAGE_ACTIVITY_DAYS)).map(t=>({kind:'thanks',date:t.createdAt||'',createdAt:t.createdAt||'',toName:t.toName||t.toDept||'どなたか',reason:t.reason||'ありがとう'}));return stepItems.concat(sent).sort((a,b)=>String(b.createdAt||b.date||'').localeCompare(String(a.createdAt||a.date||'')))}
function renderHistoryV070(list){const box=document.getElementById('v070History');if(!box)return;if(!list.length){box.innerHTML='<p class="empty-note">直近30日以内の活動はまだありません。</p>';return;}box.innerHTML=list.map(a=>{if(a.kind==='thanks')return '<article class="history-row thanks-history-row"><strong>'+v070ShortDate(a.date)+'</strong><span>❤️ '+v070Esc(a.toName)+'さんへ</span><small>'+v070Esc(a.reason)+'</small></article>';return '<article class="history-row steps-history-row"><strong>'+v070ShortDate(a.date)+'</strong><span>👟 '+v070Num(a.steps)+'歩</span></article>'}).join('')}
