const RinchanSync = (() => {
  ...
  function applyState(state) {
    if (!state) return false; if (state.syncToken) saveSyncToken(state.syncToken); if (state.unchanged === true) return false;
    if (state.user) { const current = participant() || {}; writeJson('rinchanParticipant', Object.assign({}, current, state.user)); }
    ...
    if (Array.isArray(departments)) writeJson('rinchanDepartments', normalizeDepartments(departments));
    if (state.mori || state.forest) writeJson('rinchanMoriState', state.mori || state.forest);
    return true;
  }
