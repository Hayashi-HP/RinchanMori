const RinchanNews = (() => {
  ...
  function employeeId() {
    ...
    const user = participant(); return user && (user.employeeId || user.id) ? String(user.employeeId || user.id) : '';
  }
  ...
  function notices() { const custom = readJson('rinchanNotices', null); return Array.isArray(custom) && custom.length ? custom : defaultNotices(); }
  function groupNews() {
    const custom = readJson('rinchanGroupNews', null);
    return Array.isArray(custom) && custom.length ? custom : [{ id: 'group-news-001', title: 'みんなで続ける健康習慣', body: '無理なく、できる範囲で歩数記録を続けましょう。', createdAt: new Date().toISOString(), icon: '🌳', tag: '健康' }];
  }
  ...
  function normalizeThanksItem(item, index) {
    ...
    const reason = item.reason || item.message || 'ありがとう';
  ...
  function renderNotices() {
    ...
    const read = readIds(); const rows = notices().slice().sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')));
    ...
    box.innerHTML = rows.map(item => {
      ...
      return '<article class="notice-item ' + (isRead ? 'is-read' : 'unread') + '"><span class="notice-dot">' + escapeHtml(item.icon || '🔔') + '</span><div class="notice-title"><div class="notice-meta-row"><time>' + formatDate(item.createdAt) + '</time><span class="notice-state">' + (isRead ? '既読' : '未読') + '</span>' + action + '</div><strong>' + escapeHtml(item.title || 'お知らせ') + '</strong><p>' + escapeHtml(item.body || '') + '</p></div></article>';
    }).join('');
  }
  function renderGroupNews() {
    const box = document.getElementById('groupNewsList'); if (!box) return;
  ...
  function updateBadges() { const count = unreadCount(); document.querySelectorAll('.notify-badge').forEach(badge => { badge.textContent = String(count); badge.classList.toggle('hidden', count <= 0); }); }
  async function markRead(newsId) {
    if (!newsId) return; const ids = readIds(); if (!ids.includes(newsId)) ids.push(newsId); saveReadIds(ids); renderNotices(); updateBadges();
    const id = employeeId(); if (id) applyResult(await api('markNewsRead', { employeeId: id, newsId }));
  }
  ...
  function install() { renderAll(); window.v100RenderSummary = renderSummary; window.v100RenderThanksFlowSummary = renderThanksFlowSummary; window.v100RenderThanksStories = renderThanksFlowSummary; window.v100RenderNotices = renderNotices; window.v100RenderGroupNews = renderGroupNews; window.updateNewsBadgesV051 = updateBadges; window.updateNewsRowsV051 = renderNotices; window.openNews = openNews; window.markNoticeReadV113 = markRead; }
