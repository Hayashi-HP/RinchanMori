const RinchanMypage = (() => {
  ...
  function renderBadges() {
    ...
    box.innerHTML = badges.map(badge => '<div class="badge-card ' + (badge.earned ? 'is-earned' : 'is-locked') + '"><div class="badge-mark">' + badge.icon + '</div><strong>' + badge.title + '</strong><small>' + badge.note + '</small></div>').join('');
  }
  function hideEdit() { document.querySelectorAll('.form.edit-panel').forEach(el => el.classList.add('hidden')); }
  ...
  function install() { if (!document.getElementById('mypageV070')) return; renderAll(); window.renderV070Mypage = renderAll; window.showEdit = showEdit; window.hideEdit = hideEdit; }
