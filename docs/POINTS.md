...
ポイントは、アプリを開くだけのゲームではなく、健康行動と職員同士の感謝を続けるきっかけとして使う。
...
| キー | 名称 | 初期値 | 上限 |
|---|---|---:|---|
| `initial_registration` | 初回登録 | 50りん | 初回のみ |
| `daily_open` | 1日1回アプリを開く | 1りん | 1日1回 |
| `activity_sync` | 歩数同期 | 2りん | 1日1回 |
| `daily_step_goal` | 今日の歩数目標達成 | 5りん | 1日1回 |
| `weekly_step_goal` | 週間歩数目標達成 | 20りん | 週1回 |
| `thanks_received` | ありがとう受信 | 50りん | 1日最大1回分 |
| `event_participation` | イベント参加 | 50りん | イベントごとに1回 |
...
| キー | 名称 | 初期値 | 制限 |
|---|---|---:|---|
| `limited_badge` | 限定バッジ（りん杜サポーター） | 100りん | 1人1回 |
| `rin_cafe` | りんカフェ | 500りん | 1人月1回 |
| `rinchan_goods` | 限定りんちゃんグッズ | 1,000りん | なし |
| `special_lottery` | 特別抽選応募 | 2,000りん | なし |

りんカフェは「理事長とおごり自販機で飲み物1本＋5〜10分の雑談」を想定する。
...
設定行が欠損した場合、全体または該当項目を無効・0りんとして扱う。初回セットアップ時に限り、初期設定を一括作成する。
...
ポイント付与は、利用者登録・歩数保存・ありがとう保存が成功した後の副処理として行う。
...
| action | 用途 |
|---|---|
| `getUserState` | 日次利用付与、ポイント状態取得 |
| `redeemPointReward` | 本人のご褒美交換 |
| `adminSettings` | 管理設定取得 |
| `adminSavePointSettings` | ポイント設定保存 |
...
`管理画面 → ポイント・その他設定` で以下を変更できる。

- 制度全体のON/OFF
- 各付与条件の名称、ON/OFF、付与りん
- 各ご褒美の名称、ON/OFF、必要りん

付与りんは0〜100,000、ご褒美は1〜10,000,000の整数に制限する。負数や範囲外は保存しない。
...
制度OFF中は「現在休止中」と表示し、ポイント・ご褒美の操作を隠す。
