...
- ホーム・杜カード見出しの非操作型丸アイコンを削除
- カード内リンクを文字＋矢印の `card-link` スタイルへ統一
- 操作可能要素と装飾要素の見た目を明確に分離

## v1.5.39

- 歩数記録カード見出しの非操作型プラス・履歴アイコンを削除
- 操作可能に見える装飾を整理

## v1.5.38

- 杜画面へ専用デザインシステムと共通モダンヘッダーを導入
- PCを12列グリッド、スマートフォンを1列レイアウトへ変更
- 部署マップのPC幅制限を解除し、4列表示へ拡張
- 杜だより・花だより・見どころを統一カードへ整理
- 通常画面でのりんちゃん自動挿入を停止
- 既存の杜成長・部署詳細・ありがとう・季節イベント処理を維持

## v1.5.37

- ホームと歩数記録の主要リングを週間累計・週間目標ベースへ変更
- 100%超過分を外周リングで可視化
- 目標達成後に超過歩数と実際の達成率を表示
- 選択日を含む月曜〜日曜の週間集計に対応

## v1.5.36

- 歩数記録ヒーローからりんちゃんの表示を削除
- キャラクターを常時表示せず、意味のある場面で使う方針へ調整
- ヒーローの高さと文章幅を再調整

## v1.5.35

- 歩数記録ヒーローの見出しをスマートフォンで読みやすい2行に調整
- りんちゃんの表示幅を少し縮め、文章側の表示領域を拡張
- 380px以下の端末向けにも専用の幅調整を追加

## v1.5.34

- スマートフォンで歩数記録カードと日付入力欄が横にはみ出す問題を修正
- 旧共通CSS由来のカード外側余白を専用デザインシステム側で無効化
- フォーム部品へ `min-width: 0` と最大幅制約を追加
- 歩数記録画面の不要な横スクロールを防止
