...
- ホームへ所持りんと当日の利用ポイントを表示し、パスポートの「あなたのりん」へ移動できる導線を追加
- 100りんで交換できる限定バッジ「りん杜サポーター」を追加し、1人1回のみ交換可能に設定
- 限定バッジ交換後、パスポートのバッジ図鑑へ獲得済みとして表示
- `point_transactions` を正本とする「りん」ポイント台帳を追加
- 制度全体、7つの付与条件、4つのご褒美を `app_settings` から変更できるよう拡張
- 全体・個別のON/OFF、名称、付与りん、必要りんを管理画面で編集可能に変更
...
- パスポートへ所持りん、累計獲得、直近5件、有効なご褒美、交換操作を追加
- 交換を負の取引として保存し、累計獲得を維持する方式を採用
- 500りんの「りんカフェ」を1人月1回に制限
- ポイント副処理の失敗を既存機能から分離し、サーバーエラー記録と管理者再処理APIを追加
- ポイント制度の自動テストとPC・375pxスマートフォンのレイアウト確認を追加
