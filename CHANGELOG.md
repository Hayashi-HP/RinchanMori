...
- 共通設定から後付けされていた旧CSS 14枚の自動読込を廃止し、各画面が明示したCSSだけを読み込む構成へ変更
- 管理・認証画面では不要な状態安定化JavaScriptを読み込まないよう対象画面を限定
- 同時に始まった利用者同期を1回へまとめ、ホームとパスポートの初回二重同期を防止
- 杜画面で連続していた同内容の利用者状態取得を1回へ削減
- アプリ復帰時に歩数だけを取得する軽量 `activitySnapshot` を追加
- Appleショートカットの保存応答から不要な全画面状態生成を外し、歩数保存とH判定後に早く応答するよう改善

## v1.6.0

- H制度の全体ON/OFFをPC画面の右端へ配置し、付与条件・ご褒美を2列のコンパクト表示へ改善
- 管理画面に全職員の所持Hを多い順で確認できる一覧を追加
- デイリーチェックインを専用通信なしで既存のユーザー状態取得・歩数・ありがとう処理へ同梱し、日本時間の日単位で一意化
- 歩数のショートカット入力と手入力を職員・日付単位へ統合し、通常同期は最大値、明示訂正は置換に変更
- 歩数同期Hと日次目標HのsourceIdへ職員番号と日付を含め、入力経路と関連レコードを台帳へ保存
- 日次目標を週間目標から分離し、本人が設定した1,000〜100,000歩だけを有効化
- 共通日次目標を管理画面へ追加し、初期値をOFFに設定
- パスポートと職員管理へ1日の歩数目標を追加
- ホームへ所持Hと当日の利用Hを表示し、パスポートの「あなたのH」へ移動できる導線を追加
- 100Hで交換できる限定バッジ「Hサポーター」を追加し、1人1回のみ交換可能に設定
- 表示名称をHへ統一し、「H＝5H Values・Hayashi・Hayashinableを表すシンボル」という説明を追加
- 限定バッジ交換後、パスポートのバッジ図鑑へ獲得済みとして表示
- `point_transactions` を正本とするH台帳を追加
- 制度全体、7つの付与条件、4つのご褒美を `app_settings` から変更できるよう拡張
- 全体・個別のON/OFF、名称、付与H、必要Hを管理画面で編集可能に変更
- 初回登録、日次利用、歩数同期、日次目標、週次目標、ありがとう受信、イベント参加のサーバー側付与を追加
- 同じ利用日・活動日・週・ありがとう受信日・イベントに対する二重付与を防止
- 全体OFF期間は新規付与と交換を停止し、既存台帳を保持。再開後は新しい行動から付与
- パスポートへ所持H、累計獲得H、直近5件、有効なご褒美、交換操作を追加
- 交換を負の取引として保存し、累計獲得を維持する方式を採用
- 500Hの「Hカフェ」を1人月1回に制限
- H副処理の失敗を既存機能から分離し、サーバーエラー記録と管理者再処理APIを追加
- H制度の自動テストとPC・375pxスマートフォンのレイアウト確認を追加

## v1.5.61

- ありがとうの種類を押しただけでは送信せず、選択状態として保持する方式へ変更
- 選択中の項目へ `is-selected` を付け、緑色の表示が選択に追従するよう修正
- 「この内容で送る」を追加し、送信中・送信完了・送信失敗をポップアップ内へ表示
- 送信中の全操作を無効化し、同じ相手への多重送信をクライアント側でも防止

## v1.5.60

- 杜のありがとう確認UIに、単独で完結するポップアップ用スタイルを追加
- 読み込まれていない旧HotFix CSSへ依存していたため、確認UIが通常レイアウトへ流れ込む問題を解消
- 背景、重なり順、下部メニューとの余白、スマートフォンの1列表示を明示
...
- 同じ相手への再送を7日後から可能に変更
- 自分自身へのありがとう送信を禁止
- 端末内の永久的な送信済みフラグを廃止し、サーバー同期された送信履歴で判定
- ボタン表示を「送信しました」「あと○日」「本日は2人に送信済み」に変更
- サーバー側でも日次上限・7日制限・自己送信禁止を検証し、同時送信時の競合を防止

## v1.5.58

- スマートフォンの部署詳細を杜の背景コンテナ外にある専用表示枠へ移動
- 部署詳細の閉じるボタンと「ありがとう」ボタンを確実なクリック処理へ変更
- 詳細カードと操作ボタンの重なり順・クリック領域を修正

## v1.5.57

- スマートフォンのゲスト案内文を意味のまとまりに沿った3行表示へ調整
- PCでは従来どおり1行の文章表示を維持

## v1.5.56

- ログアウト時の「はじめての方へ」を中央揃えに変更
- ログインボタンと案内文の左右余白を統一
- ゲスト案内と後続セクションの間に22pxの余白を追加

## v1.5.55

- 「ありがとうを送る」画面を共通の青・緑・白基調デザインへ刷新
- 古い個別CSSとインライン装飾を専用デザインシステムへ整理
- PCは届け先と入力フォームの2カラム、スマートフォンは1セクションずつの表示へ変更
- 絵文字ナビを共通の線画アイコンへ統一し、入力欄とボタンのタップ領域を改善
- 既存の送信先選択・メッセージ・Google Apps Script送信処理を維持

## v1.5.54

- プロフィールの項目名と内容を同じ行高にそろえ、左右の区切り線のずれを修正

## v1.5.53

- プロフィールの内容を項目名より大きな16pxへ変更し、情報の強弱を明確化

## v1.5.52

- パスポート内の概要セクションを「思い出コレクション」へ変更し、バッジ図鑑を復元
- スマートフォンのバッジ図鑑を横スクロールできる2段表示にして縦方向を抑制
- 従来のマイバッジを「達成バッジ」へ改名し、バッジ図鑑との役割を明確化
- 入職年月・勤続年数をプロフィールへ移動し、重複していた氏名の歩み表示を削除

## v1.5.51

- りんちゃんパスポート概要から重複する総歩数・ありがとう件数を削除
- 重複していたバッジ図鑑を外し、バッジ表示を「マイバッジ」へ集約
- パスポート概要を在籍の歩みとイベント参加記録に絞り、縦方向を短縮

## v1.5.50

- スマートフォンの「木の情報」で累計歩数を横幅いっぱいの主項目へ変更
- 樹齢と活動回数を下段の2列に整理し、大きな累計歩数も1行で表示

## v1.5.49

- スマートフォンの共通ヘッダー内側を各セクションと同じ左右18pxの余白へ統一
- ヘッダー背景は画面幅いっぱいのまま維持し、操作アイコンが端に寄りすぎない配置へ調整

## v1.5.48

- パスポートの「あなたの木」見出しを左揃えへ修正
- パスポート概要側の重複した木アイコンを削除
- 最近の活動を日付・曜日と歩数の左右配置へ変更
- 継続・木の情報の項目名と数値の文字サイズ・間隔を調整
- スマートフォンの共通ヘッダー背景を画面幅いっぱいへ拡張

## v1.5.47

- パスポート画面へ専用デザインシステムを導入し、旧CSSの多重読み込みを整理
- PCを全幅・2分割・3分割の共通カラムルールへ変更
- スマートフォンを1セクションずつの表示へ変更
- プロフィール・歩み・チャレンジ・バッジ・ありがとう・活動・目標を情報の優先順に再配置
- 編集フォームを共通モーダルへ統一
- 既存の同期・プロフィール編集・目標・バッジ・チャレンジ・ありがとう履歴・管理者導線を維持

## v1.5.46

- スマートフォンの杜更新アイコンを1.4秒で1回転する速度へ調整

## v1.5.45

- PCメニューの通信未読バッジを通信アイコン右上へ修正
- スマートフォンのバッジ位置は維持

## v1.5.44

- PCヘッダー付近の最終同期表示を共通コンテンツ幅の内側へ修正
- スマートフォンのヘッダーアイコン位置を画面端側へ調整
- 杜の手動更新中に更新アイコンを回転し、重複操作を防止
- 歩数記録ヘッダーのベルアイコンを削除
- 全主要画面の通信メニューへ未読・新着件数バッジを追加

## v1.5.43

- PCレイアウトを12列基準に統一し、全幅12・2分割6・3分割4の比率へ整理
- ホームをヒーロー全幅、その下を均等な2カラムへ変更
- 杜を成長・杜だよりの2カラム、部署の木・部署詳細の全幅、その下を2カラムへ変更
- 歩数記録を入力1/3・サマリー2/3・直近記録全幅へ変更
- 通信を今日の杜全幅、その下を均等な2カラムへ変更
- 今後刷新するパスポートにも同じカラムルールを適用する方針を追加

## v1.5.42

- スマートフォンの杜で、選択した部署と同じ行の直後に詳細を表示
- 選択中の部署カードを枠線で分かりやすく表示
- 部署詳細の開閉状態を支援技術へ伝える属性を追加

## v1.5.41

- 通信画面へ専用デザインシステムと共通モダンヘッダーを導入
- PCを12列グリッド、スマートフォンを重要度順の1列レイアウトへ変更
- `receivedThanksList` を画面へ追加し、ありがとうの花の受け取り機能を復活
- お知らせ・ありがとう・グループニュースの表示を整理
- 旧通信CSSの多重読み込みを停止
